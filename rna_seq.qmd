...
In @fig-pcaplot, We can see the distance between the samples gene expression,
find out the reasons for clustering of samples , investigate if there are any batch effects and add them in deseq2 while modelling.PCA is also useful to remove the outlier samples for the downstream analysis .
