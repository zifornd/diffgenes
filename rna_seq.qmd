...
This section contains the sample metadata. Sample name , condition and type or batch effect is present in the table.
...
The differential expression analysis using DESeq on DESeqDataSet object for Treated Vs Untreated condition. The various steps of DESeq includes:

1.  The estimation of size factors

2.  The estimation of dispersion values

3.  Fitting a final generalized linear model
...
This section contains the results table which has the differentially expressed genes over the particular contrast after deseq analysis.
...
contrasts <- names(params$contrasts)
...
for (contrast in contrasts){
  ...
  cat("###",contrast,"\n")
  cat(knitr::knit_print(DT::datatable(res_table, height = "20%", options = list(DT.warn.size = FALSE))))
  cat("\n\n")
}
...
This section contains summary of the results which will give an idea about the up regulated and down regulated number of genes for the particluar contrast.
...
Here is the shortlisted differentially expressed significant genes with padj < 0.05 and abs(log2FoldChange) > 1) :
...
parameters <- list(
    ...
    list(
        ...
        Value = params[[4]],
        Description = "The design for the Differntial Expression Analysis"
    ),
    list(
    ...
    list(
        ...
        Value = params[[6]],
        Description = "Colname in the metadata used as factor for Differential Expression "
    ),
    
    list(
        Parameter = names(params[7]),
        Value = names(params[[7]]),
